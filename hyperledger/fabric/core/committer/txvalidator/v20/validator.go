...
func (v *TxValidator) validateTx(req *blockValidationRequest, results chan<- *blockValidationResult) {
	...
	if env, err := protoutil.GetEnvelopeFromBlock(d); err != nil {
		...
		if common.HeaderType(chdr.Type) == common.HeaderType_ENDORSER_TRANSACTION {
			...
			fmt.Println("###### Hello from compiled fabric, Notice me senpai ######")
			fmt.Println("Example: ", chdr.ChannelId)
			fmt.Println("One more it is just header: ", payload.Header.SignatureHeader)
			fmt.Println("Block number: ", block.Header.Number)
			fmt.Println("Block Hash: ", block.Header.PreviousHash)
