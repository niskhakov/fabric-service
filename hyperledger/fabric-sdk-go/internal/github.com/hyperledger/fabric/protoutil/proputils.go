...
func CreateChaincodeProposal(typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, creator []byte, randomization *fab.RandomizationData) (*peer.Proposal, string, error) {
	return CreateChaincodeProposalWithTransient(typ, channelID, cis, creator, nil, randomization)
}

// CreateChaincodeProposalWithTransient creates a proposal from given input
// It returns the proposal and the transaction id associated to the proposal
func CreateChaincodeProposalWithTransient(typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, creator []byte, transientMap map[string][]byte, randomization *fab.RandomizationData) (*peer.Proposal, string, error) {
	...
	return CreateChaincodeProposalWithTxIDNonceAndTransient(txid, typ, channelID, cis, nonce, creator, transientMap, randomization)
}
...
func CreateChaincodeProposalWithTxIDAndTransient(typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, creator []byte, txid string, transientMap map[string][]byte) (*peer.Proposal, string, error) {
	...
	return CreateChaincodeProposalWithTxIDNonceAndTransient(txid, typ, channelID, cis, nonce, creator, transientMap, nil)
}

// CreateChaincodeProposalWithTxIDNonceAndTransient creates a proposal from
// given input
func CreateChaincodeProposalWithTxIDNonceAndTransient(txid string, typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, nonce, creator []byte, transientMap map[string][]byte, randomization *fab.RandomizationData) (*peer.Proposal, string, error) {
	...
	randomData := &peer.Randomization{
		VrfOutput: randomization.VrfOutput,
		VrfProof: randomization.VrfProof,
		LedgerHeight: uint64(randomization.LedgerHeight),
	}
	randomPayload, err := proto.Marshal(randomData)
	if err != nil {
		return nil, "", errors.Wrap(err, "error marshaling Randomization")
	}
...
func CreateProposalFromCISAndTxid(txid string, typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, creator []byte, randomization *fab.RandomizationData) (*peer.Proposal, string, error) {
	nonce, err := getRandomNonce()
	if err != nil {
		return nil, "", err
	}
	return CreateChaincodeProposalWithTxIDNonceAndTransient(txid, typ, channelID, cis, nonce, creator, nil, randomization)
}

// CreateProposalFromCIS returns a proposal given a serialized identity and a
// ChaincodeInvocationSpec
func CreateProposalFromCIS(typ common.HeaderType, channelID string, cis *peer.ChaincodeInvocationSpec, creator []byte, randomization *fab.RandomizationData) (*peer.Proposal, string, error) {
	return CreateChaincodeProposal(typ, channelID, cis, creator, randomization)
}
...
func CreateGetChaincodesProposal(channelID string, creator []byte) (*peer.Proposal, string, error) {
	...
	return CreateProposalFromCIS(common.HeaderType_ENDORSER_TRANSACTION, channelID, lsccSpec, creator, nil)
}
...
func CreateGetInstalledChaincodesProposal(creator []byte) (*peer.Proposal, string, error) {
	...
	return CreateProposalFromCIS(common.HeaderType_ENDORSER_TRANSACTION, "", lsccSpec, creator, nil)
}
